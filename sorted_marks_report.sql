...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('--- Sorted Results (Descending Marks) for Subject: ' || v_subject || ' ---');
    
    -- Cursor FOR loop to retrieve and display student name and marks
    -- The ORDER BY clause ensures the data is retrieved in descending order of marks
    FOR student_rec IN (
        SELECT
            s.name,
            r.marks
    ...
    LOOP
        -- Output the name and marks in the required format: Sneha Mishra 92
